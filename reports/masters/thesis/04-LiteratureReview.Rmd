...
chance of errors [@PacBio2023; @Allen1976].
...
(HMMs) to predict the DNA sequence, but modern basecallers have adopting neural networks
...
and is defined as the proportion of characters that match exactly between two sequences.
Sequences are considered to be in the same cluster when the pairwise sequence identity is above a predefined threshold.
When the sequence identity threshold to cluster a set of sequences is below 100%,
...
differs significantly from traditional OTU based clustering by aiming to
...
Compared to OTU clustering where identity threshold are defined *a priori*, ASVs have the advantage to be more precise,
...
In fungal metabarcdoing, taxonomic assignment is the process of classifying sequences to species or
...
but this approach is limiting because fixed thresholds that accurately delineate taxa in one part of the fungal tree
...
- ML assignment approaches
  - Ribosomal Database Project naïve Bayes classifier
  - emu
