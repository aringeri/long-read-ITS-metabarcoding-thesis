...
We have found that using the lowest cluster size threshold gives the highest classification accuracy at the cost of overestimating diversity.
...
This QC approach is stringent and could bias against some taxa in a real world scenario, and we aim to make this configurable in the pipeline for end users.
The reads for the mock dataset were generated in 2022 and were not able to use the dorado's latest basecalling model (v5.0.0 at the time of writing).
...
(for example: *Pucciniales* in @fig-splittingPuccinia).
<!-- Cryptococcus and sclerotiniaceae -->
...
but had limitations when comparing clustering methods.
...
We found that the number of OTUs was a good initial measure for comparing clustering methods as it was relatively quick to compute.
However, after performing taxonomic assignments, we found that using the number of OTUs to measure species diversity tends to overestimate the number of species present because many OTUs shared the same species-level classifications (@fig-fixed-min-cluster-size-even).
...
Our customised NanoCLUST approach was unsuitable for clustering large datasets due to computational limitations.
In our case, the soil sample dataset contained 1.8 million reads to be clustered and attempting to run UMAP and HDBSCAN resulted much higher memory use than was possible with our system (using up to 128GB RAM).
...
In our mock communities, in the best cases we observed a classification rate of ~95% and genera-level precision of 82% to 85%.
...
This went against our expectations that low-abundance OTUs were mostly erroneous and would reduce classification precision.
We expect that these small OTUs are the product of sequencing errors that make them different from major OTUs but still contain sequences that are similar to the 'correct' reference database entry.
...
However, we expect that there was significant biological variation present within OTUs which introduced noise into the resulting consensus sequence.
This issue was exacerbated when using consensus sequences and VSEARCH at 97% identity as only up to 35% of reads were given classifications.
...
If we were to pursue consensus sequences in the future,
...
- limitation of using a single sequence to classify OTU


The real-world soil case study display considerably different patterns of fungi than our mock scenarios.
