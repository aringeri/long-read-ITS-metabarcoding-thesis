...
We have tested the impact of a minimum cluster size threshold and found that the lowest cluster size threshold gave the highest classification accuracy at the cost of overestimating species diversity.
...
This QC approach may be overly conservative and bias against some taxa if applied generally in real world scenarios.
...
We have demonstrated that clustering with VSEARCH and our NanoCLUST implementation produced comparable results and
...
Although if using NanoCLUST to cluster, we would not recommend high values of the `min_cluster_size` parameter due to merging of unrelated taxa as discussed above and shown in @fig-uneven-min-cluster-thresh.
