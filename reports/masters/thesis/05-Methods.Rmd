...
In order to understand the behaviour of our pipeline under different sampling scenarios, a mock dataset was used based on sequencing data from 65 known fungal isolates.
We simulated data that might be encountered when sampling a complex fungal community, by controlling
the relative proportion of reads from each fungal isolate using various scenarios:

1) Even abundance where all fungal isolates have the same relative abundance of read count.
2) Uneven scenario where a subset of fungal isolates have a much lower abundance relative to the other isolates in the mock sample.
...
After the raw reads from the fungal isolates had been processed as described above, we had a pool of high quality full ITS sequences which we used to simulate different community structures.
...
The first mock scenario simulated an even community structure where every fungal isolate has an equal abundance in terms of number of reads.
...
The species *Trichomonascus ciferrii*, *Erysiphe necator*, *Eutypa lata*, *Penicillium chrysogenum* and *Candida zeylanoides* were randomly selected as the low abundance species.
