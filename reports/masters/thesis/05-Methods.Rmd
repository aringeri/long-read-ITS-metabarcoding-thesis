...
This limit of 2500 reads was chosen as I wanted to test scenarios with library sizes of up to 150000 reads but with this limit we could also retain the majority of the samples (see @fig-readCountDistributionBySample for the distribution of read counts per sample, and @tbl-samplesheet for read counts).
...
:::{custom-style="Caption"}
The process for generating mock communities with equal abundance. Equal numbers of reads are selected from the set of quality filtered reads of each fungal isolate then combined into a single library. Each library is processed by the pipeline individually.
:::
...
A minimum OTU size threshold of 0.1% of the library size was used to filter out low-abundance OTUs and more closely estimate the number of species (see @sec-cluster-results for the determination of this threshold).

The second approach clustered sequences by similarity in k-mer signatures.
We followed the approach used by the NanoCLUST tool [@rodriguez-perezNanoCLUSTSpecieslevelAnalysis2021; @langsiriTargetedSequencingAnalysis2023].
...
See @sec-cluster-results for determination of the 'min_cluster_size' parameter.
