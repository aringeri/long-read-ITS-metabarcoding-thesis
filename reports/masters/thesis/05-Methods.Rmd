...
This limit of 2500 reads was chosen in order to test scenarios with library sizes of up to around 150000 reads.
With this limit we could also retain the majority of the samples (see @fig-readCountDistributionBySample and @tbl-samplesheet for the distribution of read counts per sample).
...
As it is unlikely for a 'real world' environmental sample to yield a read distribution in Scenario 1 where all strains have the same number of reads,
we explored limited situations where the number of reads between isolates was unequal.
For this scenario, five species were subsampled at low abundance, while a remaining 50 species were subsampled at a higher abundance (@fig-unevenSamplingDiagram).
The low-abundance species *Trichomonascus ciferrii*, *Erysiphe necator*, *Eutypa lata*, *Penicillium chrysogenum*, *Candida zeylanoides*,
...
Each library was processed by the pipeline individually.
:::
...
- removed duplicate species
  - excluded: Austropuccinia_psidii, Cryptococcus_gattii_VG_III, Cryptococcus_neoformans_VNI in addition to those from Scenario 1
...
The 'cluster_selection_epsilon' parameter used in HDBSCAN was set to 0.5 as used in [@langsiriTargetedSequencingAnalysis2023], while the multiple '`min_cluster_size`' parameters were tested and explored in @sec-cluster-results.
