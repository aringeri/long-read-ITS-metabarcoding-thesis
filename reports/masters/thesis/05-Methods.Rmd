...
In order to validate the pipeline, a mock dataset has been created to simulate scenarios where multiple fungal species are present in the same sample.
Using reads from ONT sequencing of 65 known fungal isolates, we are able to control the proportions of species for each simulated scenario.
...
To ensure consistent taxonomy between the fungal isolate and the reference database used for classification, the recorded species name of each sample was searched for in the 2024 UNITE reference database. Species that could not be found were either misspelled or had a older taxonomic synonym. For samples that could not be found in the reference database, names were manually updated using current names found in both indexfungorum.org and the reference database.

Samples that had less than a total of 2500 reads after basecalling, trimming and filtering were removed from the analysis leaving 60 samples. They were: ...
...
- Even distribution
  ...
  - 56 species (two Austropuccinia strains, two cryptococcus gattii, two cryptococcus neoformans)
- subsampling
  - seqtk v1.4
