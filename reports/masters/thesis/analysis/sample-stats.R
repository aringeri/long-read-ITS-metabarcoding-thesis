...
gather_qc_stats <- function(config) {
  post_qc_stats <- data.frame(bc = NULL, qcd_reads = NULL)

  qc_dir <- glue('{config$experiment_path}/QC/05-post_chimera_filtering/FULL_ITS/nanoplot')
...
qc_stats <- gather_qc_stats(config)
...
fmted <- read.csv('../../../../experiments/66-fungal-isolate-ONT/sample-stats.tsv', sep='\t') %>%
  ...
  rename(Barcode = 'BC', Sample = 'sample',  'Number of raw reads'=num_seqs, 'Number of reads after QC'=qcd_reads)
