...
load_loss_stats <- function() {
  ...
  for (depth in sample_depths) {
    ...
    for (rep in reps) {
      ...
      nanoclust_stats <- read.csv(glue('../../../../experiments/66-fungal-isolate-ONT/outputs/isolate-even-reps-08-02/hdbscan_clustering_stats/FULL_ITS/{depth}/{rep}/min_cluster_size_stats.tsv'), sep='\t')
...
otu_counts <- combined_stats %>%
  ...
  geom_hline(aes(yintercept = 59, linetype='59'))+#, show.legend =TRUE) +
