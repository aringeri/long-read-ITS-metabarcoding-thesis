...
# problematic <- readRDS('../../../../experiments/66-fungal-isolate-ONT/outputs/isolate-even-reps-07-24/phyloseq/FULL_ITS/2000/5/all_samples/all_samples.phyloseq.rds')
...
load_otus <- function(filter_thresh = c(0, 0.006)) {
  ...
  sample_depths <- list.dirs(
    '../../../../experiments/66-fungal-isolate-ONT/outputs/isolate-even-reps-08-02/phyloseq/FULL_ITS',
  ...
  for (depth in sample_depths) {
    reps <- list.dirs(
      glue('../../../../experiments/66-fungal-isolate-ONT/outputs/isolate-even-reps-08-02/phyloseq/FULL_ITS/{depth}/'),
      recursive = FALSE, full.names = FALSE
    )
    ...
    for (rep in reps) {
      otus <- readRDS(glue('../../../../experiments/66-fungal-isolate-ONT/outputs/isolate-even-reps-08-02/phyloseq/FULL_ITS/{depth}/{rep}/all_samples/all_samples.phyloseq.rds'))
...
taxa_counts <- load_otus(c(0, 0.01, 0.005, 0.001, 0.0005, 0.0001))
...
load_nanoclust_stats <- function(filter_thresh = c(0, 0.0012)) {
  stopifnot(length(filter_thresh) > 0)
  df <- data.frame(matrix(ncol = 3, nrow = 0))
  colnames(df) <- c("sample_depth"
    ,"thresh"
    ,'nOTUs')
  sample_depths <- list.dirs(
    '../../../../experiments/66-fungal-isolate-ONT/outputs/isolate-even-reps-08-02/hdbscan_clustering_stats/FULL_ITS/',
    recursive = FALSE, full.names = FALSE) %>%
    as.numeric() %>%
    sort()
  ...
  for (depth in sample_depths) {
    reps <- list.dirs(
      glue('../../../../experiments/66-fungal-isolate-ONT/outputs/isolate-even-reps-08-02/hdbscan_clustering_stats/FULL_ITS/{depth}/'),
      recursive = FALSE, full.names = FALSE
    )
    stopifnot(length(reps) > 0)

    for (rep in reps) {
      stats <- read.csv(glue('../../../../experiments/66-fungal-isolate-ONT/outputs/isolate-even-reps-08-02/hdbscan_clustering_stats/FULL_ITS/{depth}/{rep}/min_cluster_size_stats.tsv'), sep = '\t', stringsAsFactors = FALSE)
...
otu_count_nanoclust <- load_nanoclust_stats(c(0, 0.0006, 0.0012, 0.0050999999999999995, 0.0072, 0.01005)) %>%
  mutate_at(vars(sample_depth), \(d) d*60) %>%
  ggplot(
